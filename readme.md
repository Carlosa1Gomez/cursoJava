...
`double raiz =` ahora le asignamos a la variable el parámetro de la clase que en
este caso es de un raíz cuadrada y el parámetro es: `Math.sqrt(9);` aquí
estamos calculando la raíz cuadrada de 9. Si imprimimos el valor de esta
...
Este condicional evalúa la condición (variable) que le pasemos y si esa condición
...
agregándole después del *else* un *if* y así crea la estructura del *if* nuevamente;
...
En este momento también vamos a ver los operadores de comparación ya que son
muy utilizados en los condicionales. Estos operadores son:

* `<>`: Estos signos los habrás visto en el colegio ya que en matemáticas se
  suelen usar, son los signos de mayor y menor y nos permite comparar si un
  valor es mayor o menor a otro.

* `==`: Estos dos iguales nos permiten comparar si el valor es igual a otro,
  como ya sabrás no utilizamos un solo = porque este lo usamos para asignar
  valores a las variables.

* `<= >=`: Y también podemos comparar usando los dos signos para que incluya el
  número, ejemplo: i <= 24 entonces aquí el valor de i puede ser menor a 24 o
  ser 24.

Ahora esto lo podemos usar en el condicional *if*, a continuación un ejemplo:
...
El resultado de este condicional será verdadero por lo tanto se ejecutará el
primer código, y será verdadero hasta que *numero* valga 10, cuando el *numero*
valga 11 será negativo por lo tanto se ejecutará el segundo código.
...
de control hay muchos programadores que no le gustan pero para menús con números
...
se ejecutará el que cumple la condición hasta el que tenga un break entonces
...
Los ciclos nos permite repetir código las veces que necesitemos, se usan mucho
y cuando trabajamos con arreglos también los usaremos bastante.

### Ciclo While

El ciclo while (en español mientras) es una estructura de control que evalúa
una condición y si esta se cumple ejecuta el código hasta que no se cumpla. La
estructura básica del while es:
...
Entonces como vemos mientras se cumpla la condición se ejecuta ese código,
cuando se deje de cumplir para, vamos a hacer un ejemplo:
...
public class ciclos {
  ...
  public static void main(String[] args){
    ...
    while (i <= numero) { // Evaluamos si i es <= numero

      System.out.println(i); //imprimimos el valor de i 

      i++; // Aumentamos el valor de i, si no lo hacemos entonces imprimirá 0 infinitamente
...
El resultado de este código es una lista del 0 al 10 aunque si cambiamos el
valor de *numero* entonces será del 0 al valor de *numero*. Esta es una forma
fácil de mostrar lo que hace while pero tiene muchos usos.

### Ciclo Do While

Este ciclo cumple la misma función que el while solo que en este el código se
ejecuta al menos una vez en cambio en el while si la condición no se cumple
entonces el código no se ejecuta. La estructura de este ciclo es:
...
Aquí hará lo mismo que en el anterior solo que así no se cumpla la condición el
código se ejecuta una vez.

### Ciclo For

El ciclo for hace lo mismo que el ciclo while pero con menos código y este ciclo
es el más usado y querido entre los programadores y su estructura es la siguiente:
...
Ya con estas simples líneas de código podemos hacer lo mismo que en el while,
como decía este ciclo es de los más usados por lo simple y rápido que se puede
hacer. Recuerda que si en vez de sumar un número quieres restarlo entonces el
*i++* lo cambias por *i--*.

## Arreglos o Arrays

Los arreglos son estructuras que nos permiten almacenar varios datos del mismo
tipo y el tamaño de este arreglo se define cuando se declara y después no se
puede cambiar. Estos arreglos se declaran de la siguiente manera:

`tipoVariable[] nombreVariable = new tipoVariable[cantidadDatos];`

De esta forma declaramos el arreglo y después le podemos ingresar un dato de la
siguiente manera: `nombreVariable[lugarDato] = valor;` debemos recordar que los
arreglos en la posición que empiezan es en la 0 por lo tanto si tengo tres datos
en el arreglo el orden es 0, 1, 2, al principio puede ser confuso pero con el
tiempo te vas acostumbrando. Estos son arreglos unidimensionales ya que hay
arreglos de más dimensiones. Vamos a ver un ejemplo del eso de los arreglos:
...
public class arreglo {
        ...
        public static void main(String[] args){
          ...
          //String[] coloresPrimarios = new String[3];
          String[] coloresPrimarios = {"amarillo", "azul", "rojo"};
        

          for (int i = 0; i < coloresPrimarios.length; i++) {

            System.out.println(coloresPrimarios[i]);
          }
        }
...
Aquí estamos guardando en el arreglo de nombre coloresPrimarios los
colores amarillo, azul, y rojo. Después con el ciclo for estamos mostrando
el contenido de ese arreglo.
